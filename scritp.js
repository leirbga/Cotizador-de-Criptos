...
datosIngresados.addEventListener("submit", async (evento) => {
  ...
  const monedaSeleccionada = [...selectorMoneda.children].find(
    (option) => option.selected,
  ).value;
  const criptoSeleccionada = [...selectorCripto.children].find(
    (option) => option.selected,
  ).value;
  ...
  try {
    ...
    const cotizador = await (
      await fetch(
        `https://min-api.cryptocompare.com/data/pricemultifull?fsyms=${criptoSeleccionada}&tsyms=${monedaSeleccionada}`,
      )
    ).json();
    const precioCripto =
      cotizador.DISPLAY[criptoSeleccionada][monedaSeleccionada].PRICE;
    const precioAlto =
      cotizador.DISPLAY[criptoSeleccionada][monedaSeleccionada].HIGH24HOUR;
    const precioBajo =
      cotizador.DISPLAY[criptoSeleccionada][monedaSeleccionada].LOW24HOUR;
    const variacion24H =
      cotizador.DISPLAY[criptoSeleccionada][monedaSeleccionada].CHANGEPCT24HOUR;
    const puedesComprar =
        montoValor /
        cotizador.RAW[criptoSeleccionada][monedaSeleccionada].PRICE;
    ...
    if (montoValor != "") {
      informacion.innerHTML = informacion.innerHTML +`
